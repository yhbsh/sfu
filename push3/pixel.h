...
static inline void fill_pattern_game_of_life4( int width, int height, uint8_t *Y, const int sY, uint8_t *U, const int sU, uint8_t *V, const int sV, int pts) { // --- persistent simulation state ---
    static int init = 0;
    static uint8_t *grid = NULL;
    static uint8_t *next = NULL;
    static int gw = 0, gh = 0;

    // initialize grid on first run or size change
    if (!init || gw != width || gh != height) {
        gw = width;
        gh = height;
        ...
        next = (uint8_t*)calloc(gw * gh, 1);

        // seed pattern: random-ish but deterministic
        for (int y = 0; y < gh; ++y) {
            for (int x = 0; x < gw; ++x) {
                grid[y * gw + x] = ((x * 123 + y * 17) ^ pts) & 1;
            }
        }
    ...
    // --- render into YUV 4:2:0-ish buffers ---
    for (int y = 0; y < gh; ++y) {
        uint8_t *rowY = Y + y * sY;
        int sy2 = y >> 1;
        uint8_t *rowU = U + sy2 * sU;
        uint8_t *rowV = V + sy2 * sV;

        for (int x = 0; x < gw; ++x) {
            uint8_t cell = grid[y * gw + x];
...
static inline void fill_pattern_cellular_automata( int width, int height, uint8_t *Y, const int sY, uint8_t *U, const int sU, uint8_t *V, const int sV, int pts) {
    ...
    // Start row seed:
    // Use pts just to vary the initial condition.
    for (int x = 0; x < width; x++) {
        Y[x] = (uint8_t)((((x + pts) % 7) == 0) ? 255 : 0);
    }

    // Generate each next row in Y using Rule 30
    for (int y = 1; y < height; y++) {
        uint8_t *prev = Y + (y - 1) * sY;
        uint8_t *curr = Y +  y      * sY;

        for (int x = 0; x < width; x++) {
            uint8_t left  = prev[(x == 0) ? (width - 1) : (x - 1)] > 127;
            uint8_t self  = prev[x] > 127;
            uint8_t right = prev[(x == width - 1) ? 0 : (x + 1)] > 127;

            uint8_t neighborhood = (left << 2) | (self << 1) | right;

            // Rule 30 bit: (0b00011110 >> neighborhood) & 1
            uint8_t next = (0x1E >> neighborhood) & 1;

            curr[x] = next ? 255 : 0;
        }
    }

    // Fill U and V with smooth gradients modulated by Y
    for (int y = 0; y < height; y++) {
        uint8_t *Uy = U + y * sU;
        uint8_t *Vy = V + y * sV;
        uint8_t *Yy = Y + y * sY;
        ...
        for (int x = 0; x < width; x++) {
            // Use Y as a mask to make chroma flicker based on automaton
            uint8_t mask = Yy[x] > 127 ? 1 : 0;

            Uy[x] = (uint8_t)((((x * 3) + (y * 2) + pts) & 255) >> mask);
            Vy[x] = (uint8_t)((((x * 5) + (y * 7) + pts) & 255) >> mask);
        }
    }
}

static inline void fill_yuv(int width, int height, uint8_t *rgb, int pts) {
    // Smooth time factor
    float t = pts * 0.001f;

    // Smoothly moving gradient offsets
    float u_offset = fmodf(t * 50.0f, 255.0f);  // horizontal drift
    float v_offset = fmodf(t * 30.0f, 255.0f);  // vertical drift

    for (int y = 0; y < height; ++y) {
        // Vertical gradient component (0–255)
        float v_grad = (float)y * 255.0f / (height - 1);

        for (int x = 0; x < width; ++x) {
            // Horizontal gradient component (0–255)
            float u_grad = (float)x * 255.0f / (width - 1);

            // Smooth drifting gradient
            uint8_t U = (uint8_t)(u_grad + u_offset);
            uint8_t V = (uint8_t)(v_grad + v_offset);
